...
def train_and_predict(data):
    ...
    best_reg, y_pred_reg = train_model(XGBRegressor(random_state=42, objective='reg:squarederror'),
    ...
    best_cls, y_pred_cls = train_model(XGBClassifier(random_state=42, use_label_encoder=False, eval_metric='mlogloss'),
...
def forecast_next_date(df, anchor_date, site):
    ...
    # Regression forecast using XGBRegressor
    print("Forecasting DA Levels using XGBRegressor...")
    ...
    reg_model = XGBRegressor(random_state=42, n_estimators=100, max_depth=None, objective='reg:squarederror')
    reg_model.fit(X_train_reg_processed, df_train['DA_Levels'])
    y_pred_reg = reg_model.predict(X_test_reg_processed)

    # Classification forecast using XGBClassifier
    print("Forecasting DA Category using XGBClassifier...")
    ...
    cls_model = XGBClassifier(random_state=42, n_estimators=100, max_depth=None, use_label_encoder=False, eval_metric='mlogloss')
