...
def process_streamflow(url, temp_dir):
    ...
    # Create biweekly periods (every 2 weeks)
    # First, get the offset starting from the first date of the dataset
    if not df.empty:
        min_date = df['Date'].min()
        # Create a reference point for biweekly periods
        ref_date = min_date - pd.Timedelta(days=min_date.dayofweek)  # Start from the beginning of a week
        df['biweekly_period'] = ((df['Date'] - ref_date).dt.days // 14)
        df['biweekly_period_start'] = ref_date + pd.Timedelta(days=14) * df['biweekly_period']
        
        # Group by biweekly periods
        biweekly_flow = df.groupby('biweekly_period_start')['Flow'].mean().reset_index()
        biweekly_flow.rename(columns={'biweekly_period_start': 'Date'}, inplace=True)
        return biweekly_flow[['Date', 'Flow']].sort_values('Date')
...
def process_da(da_files_dict):
    """Processes DA data from Parquet files, returns biweekly aggregated DataFrame."""
    print("\n--- Processing DA Data ---")
    data_frames = []
    ...
    for name, path in da_files_dict.items():
        ...
        try:
            ...
            # Instead of ISO week, create biweekly periods
            if not df.empty:
                min_date = df['Parsed_Date'].min()
                # Create a reference point for biweekly periods
                ref_date = min_date - pd.Timedelta(days=min_date.dayofweek)  # Start from the beginning of a week
                df['biweekly_period'] = ((df['Parsed_Date'] - ref_date).dt.days // 14)
                df['biweekly_period_start'] = ref_date + pd.Timedelta(days=14) * df['biweekly_period']
                
                # Group by biweekly period AND site
                biweekly_da = df.groupby(['biweekly_period_start', 'Site'])['DA_Levels'].mean().reset_index()
                biweekly_da.rename(columns={'biweekly_period_start': 'Date'}, inplace=True)
                
                data_frames.append(biweekly_da[['Date', 'DA_Levels', 'Site']])
                print(f"    Successfully processed {len(biweekly_da)} biweekly DA records for {name}.")
    ...
    if not final_da_df.empty:
         final_da_df = final_da_df.groupby(['Date', 'Site'])['DA_Levels'].mean().reset_index()
    print(f"Combined DA data shape: {final_da_df.shape}")
    return final_da_df


def process_pn(pn_files_dict):
    """Processes PN data from Parquet files, returns biweekly aggregated DataFrame."""
    print("\n--- Processing PN Data ---")
    data_frames = []
    ...
    for name, path in pn_files_dict.items():
        ...
        if not df.empty:
            ...
            # Group by biweekly period AND site
            biweekly_pn = df.groupby(['biweekly_period_start', 'Site'])['PN_Levels'].mean().reset_index()
            biweekly_pn.rename(columns={'biweekly_period_start': 'Date'}, inplace=True)
            
            data_frames.append(biweekly_pn[['Date', 'PN_Levels', 'Site']])
            print(f"  Successfully processed {len(biweekly_pn)} biweekly PN records for {name}.")

    final_pn_df = pd.concat(data_frames, ignore_index=True)
    # Add a final group-by after concat
    if not final_pn_df.empty:
        final_pn_df = final_pn_df.groupby(['Date', 'Site'])['PN_Levels'].mean().reset_index()
    print(f"Combined PN data shape: {final_pn_df.shape}")
    return final_pn_df

def generate_compiled_data(sites_dict, start_dt, end_dt):
    """Generate base DataFrame with all Site-Biweekly combinations"""
    print(f"  Generating biweekly entries from {start_dt.date()} to {end_dt.date()}")
    
    # Create biweekly date range (every two weeks on Monday)
    biweekly_dates = []
    current_date = start_dt
    
    # Adjust start_dt to a Monday if it's not
    if start_dt.dayofweek != 0:  # 0 is Monday
        days_to_add = 7 - start_dt.dayofweek
        current_date = start_dt + pd.Timedelta(days=days_to_add)
    
    # Generate biweekly dates
    while current_date <= end_dt:
        biweekly_dates.append(current_date)
        current_date += pd.Timedelta(days=14)  # Add two weeks
    ...
    for site, coords in sites_dict.items():
        ...
        site_df = pd.DataFrame({'Date': biweekly_dates, 'Site': normalized_site, 'lat': lat, 'lon': lon})
        df_list.append(site_df)
        
    compiled_df = pd.concat(df_list, ignore_index=True)
    print(f"  Generated base DataFrame with {len(compiled_df)} site-biweekly rows.")
...
def compile_data(compiled_df, oni_df, pdo_df, streamflow_df):
    ...
    # Merge Streamflow data (which is now also biweekly)
    ...
    compiled_df = pd.merge_asof(
        ...
        on='Date',
        direction='backward',
        tolerance=pd.Timedelta('14days')  # Changed from 7 to 14 days to match biweekly frequency
    )
...
def compile_da_pn(lt_df, da_df, pn_df):
    ...
    da_df_copy['Date'] = pd.to_datetime(da_df_copy['Date'])
    ...
    pn_df_copy['Date'] = pd.to_datetime(pn_df_copy['Date'])
